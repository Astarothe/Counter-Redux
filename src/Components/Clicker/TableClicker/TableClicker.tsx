...
export const TableClicker: React.FC<TableClickerType> = React.memo(
    ({error, currentValue, onChange, changeMode, maxValue, minValue}) => {
        ...
        const pressSetText = "enter values and press 'set'"
        const incorrectValueText = "incorrect value"
        const increment = 1;
        ...
        const currentText = !changeMode ? currentValue : error ? incorrectValueText : pressSetText
        ...
        return (
            <div className={s.container}>
                <div className={s.wrapperText}>
                    <p className={`${maxValue === currentValue && !changeMode ? s.red + " " + s.text : textStyle}`}>{currentText}</p>
                </div>
